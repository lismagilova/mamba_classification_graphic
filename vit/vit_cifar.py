...
class VisionTransformer(nn.Module):
    def __init__(self, img_size=32, patch_size=4, in_channels=3, num_classes=10,
        ...
        self.patch_proj = nn.Sequential(
            ...
            PermuteAndReshape((-1, patch_dim, num_patches), (0, 2, 1)),
...
def benchmark_inference_speed(model, loader, device, num_warmup_batches=5, num_benchmark_batches=20):
    ...
    with torch.no_grad():
        for i, (images, _) in enumerate(loader):
            if i >= num_warmup_batches + num_benchmark_batches: break
    ...
    for epoch in range(epochs):
        ...
        if val_acc > best_val_acc:
            ...
            torch.save(model.state_dict(),
                       f'best_{model_name.lower()}_cifar10.pth')
...
def main():
    ...
    stats_vit, trained_vit_model = run_training_experiment(
        "ViT",
        VisionTransformer,
        vit_config,
        train_loader,
    ...
    if device.type == 'cuda':
        ...
        print("\nОчищена память GPU после ViT.\n")

    print("\n\n--- Сводка по эксперименту (CIFAR-10) ---")
    if all_stats_data:
        vit_stats = all_stats_data[0]
        print(f"Модель: {vit_stats['model_name']}")
        metrics_display_names = {
            "params": "Параметры",
            "total_train_time_sec": "Общее время обучения (сек)",
            "avg_epoch_time_sec": "Среднее время/эпоха (сек)",
            "peak_gpu_mem_MB": "Пик GPU памяти (обуч., МБ)",
            "test_accuracy": "Точность на тесте",
            "avg_time_per_batch_ms": "Инференс (мс/батч)",
            "throughput_img_per_sec": "Инференс (изобр/сек)"
        }
        for key, display_name in metrics_display_names.items():
            if key in vit_stats:
                value = vit_stats[key]
                if value is None or (isinstance(value, str) and value.lower() == "n/a (cpu)"):
                    value_str = "N/A"
                elif key == "params":
                    value_str = f"{value:,}"
                    ...
                    value_str = str(value)
                print(f"{display_name:<35}: {value_str}")
    else:
        print("Нет данных для отображения.")

    print("\nЭксперимент с ViT (CIFAR-10) завершен.")
    print("Графики обучения (потери и точность) и визуализация предсказаний были показаны выше.")
    print("Статистика производительности и точности модели ViT представлена в сводке.")
